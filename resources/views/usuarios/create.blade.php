...
@section('title', __('usuarios.create'))

@section('content')
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">{{ __('usuarios.create') }}</div>
            <form method="POST" action="{{ route('usuarios.store') }}" accept-charset="UTF-8">
                ...
                <div class="card-footer">
                    <input type="submit" value="{{ __('usuarios.create') }}" class="btn btn-success">
                    <a href="{{ route('usuarios.index') }}" class="btn btn-link">{{ __('app.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
